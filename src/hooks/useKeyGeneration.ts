...
export interface UseKeyGenerationReturn {
  ...
  handleGenerateKeys: (
    ...
    authenticatorType?: false | 'platform' | 'cross-platform',
...
export const useKeyGeneration = (): UseKeyGenerationReturn => {
  ...
  const handleGenerateKeys = async (
    ...
    onError: (error: string) => void,
    setLoading: (loading: boolean) => void,
    authenticatorType: false | 'platform' | 'cross-platform' = false,
    loginPassword?: string
  ) => {
    const useHardwareStorage = authenticatorType !== false;
    ...
    try {
      ...
      const hasPassphrase = passphrase && passphrase.length >= AUTH_CONFIG.passphrase.minLength;
      ...
      if (useHardwareStorage) {
        ...
        try {
          ...
          const nickname = authenticatorType === 'platform' ? 'This Device' : 'Security Key';
          const { keyData: credential, signature } = await registerHardwareKey(user.uid, user.email || '', nickname, authenticatorType || 'cross-platform');
          ...
          if (!hasPassphrase) {
            ...
            await updateUserProfile(user.uid, {
              encryptedPrivateKey: backendService.utils.deleteField() as any
            });
